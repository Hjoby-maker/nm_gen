...
class FamilyRepositoryImpl implements FamilyRepository {
  ...
  Future<Family> addFamily(Family family) async {
    ...
    final savedModel = await localDataSource.insertFamily(model);
    return savedModel.toDomain();
  }

  @override
  Future<Family?> getFamily(String id) async {
    final model = await localDataSource.getFamily(id);
    return model?.toDomain();
  }

  @override
  Future<List<Family>> getAllFamilies() async {
    final models = await localDataSource.getAllFamilies();
  ...
  Future<Family> updateFamily(Family family) async {
    final model = FamilyModel.fromDomain(family);
    final updatedModel = await localDataSource.updateFamily(model);
  ...
  Future<List<Family>> getFamiliesByPerson(String personId) async {
    final models = await localDataSource.getFamiliesByPerson(personId);
    return models.map((model) => model.toDomain()).toList();
  }

  @override
  Future<List<Family>> getFamiliesAsParent(String personId) async {
    final models = await localDataSource.getFamiliesAsParent(personId);
    return models.map((model) => model.toDomain()).toList();
  }
